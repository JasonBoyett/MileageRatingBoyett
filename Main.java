...
public class Main {// memory hook
    ...
    protected enum RATING {// a set of enumerated values to represent the different ratings a car can have
                           // and an error
    ...
    public static void main(String[] args) throws NumberFormatException, HeadlessException, Exception {// memory hook
        ...
        String carName = String.valueOf(JOptionPane.showInputDialog(null, "what kind of car do you have?", null,
                JOptionPane.INFORMATION_MESSAGE, car, null, ""));// a JOptionPane that gets the name of the users car

        try {// this try block outputs various JOptionPanes depending on the rating of the
             // user's mpg value
            RATING rating = getRATING(Double.parseDouble(String.valueOf(JOptionPane.showInputDialog(null,
                    "What is your car's MPG?", null, JOptionPane.INFORMATION_MESSAGE, car, null, ""))));// a JOptionPane
                                                                                                        // that gets the
                                                                                                        // user's mpg
            if (rating == RATING.Error) {// if the RATING comes back as an Error then an error message is displayed
                ...
                JOptionPane.showMessageDialog(null, "your " + carName + " has excelent gas milage", "Excelent",
                        JOptionPane.INFORMATION_MESSAGE, car);
            } else if (rating == RATING.Poor) {
                JOptionPane.showMessageDialog(null, "your " + carName + " has poor gas milage", "Poor",
                        JOptionPane.INFORMATION_MESSAGE, car);
            } else if (rating == RATING.Fair) {
                JOptionPane.showMessageDialog(null, "your " + carName + " has fair gas milage", "Fair",
                        JOptionPane.INFORMATION_MESSAGE, car);
            } else if (rating == RATING.Good) {
                JOptionPane.showMessageDialog(null, "your " + carName + " has good gas milage", "Good",
                        JOptionPane.INFORMATION_MESSAGE, car);
            }
    ...
    public static RATING getRATING(double mpg) {// takes the user's mpg rating and gives it a rating that is then
                                                // returned. If the mpg rating is 0 or less this method returns Error
        try {
            if (mpg > 0 && mpg < 20) {
                return RATING.Poor;
